...
async fn single_ip_check(addr: &SocketAddr) -> std::io::Result<StatusPayload> {
    ...
    let send_time = util::now_timestamp();
    stream.write_all(&buf).await?;
    log::trace!("Legacy query sent, packet length = {}", packet_len);

    let mut recv_buffer = [0u8; 3];
    timeout(timeout_time, stream.read_exact(&mut recv_buffer)).await??;
    let ping = util::now_timestamp() - send_time;
    ...
    log::trace!("Legacy query received: {}", str);
